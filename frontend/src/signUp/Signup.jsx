...
useEffect(()=>{
  if(emailAuth && firstName!=='' && mobileNum.length>9){
    setBasicDetailsBtn(true);
  }else{
    setBasicDetailsBtn(false);
  }
}, [emailAuth, firstName, mobileNum])
...
axios.post('/send_email_otp',{emailId:emailId, mobileNum:mobileNum})
.then((res)=>{
  Swal.fire({
...
const finalCreate=()=>{
axios.post('/final_user_signup', {firstName:firstName, lastName:lastName, mobileNum:mobileNum, emailId:emailId, password:password, confirmPassword:confirmPassword, houseNo:houseNo, street:street, city:city, pincode:pincode })
.then((res)=>{
  if(res.data===200){
  Swal.fire({
  ...
  console.log(err);
    ...
    return(
<div className='container-fluid mainBodyDiv' style={{height:windowHeight, width:'100%', backgroundColor:'#BD8F6E'}}>

<div className='main_div col-md-8 offset-md-2 col-sm-10 offset-sm-1'>
...
{ basicDetails && (
<div className='row row_sections p-2 mb-0 pb-0'>
...
onChange={(element)=>{
  ...
  if(element.currentTarget.value.length<10){
    element.currentTarget.classList.add('border-bage');
  }else{
    element.currentTarget.classList.remove('border-bage');
  }
}}
onBlur={(element=>removeInputStyle(element))}
...
onKeyDown={(elem)=>{
  ...
  if(num_leng===0 && elem.key !=='6' && elem.key !=='7' && elem.key !=='8' && elem.key !=='9'){
    elem.preventDefault();
  }
...
<button className='disabledSignUpButton buttonCommonText rounded-littleSharp px-4 py-1'>Confirm Email</button>
</div>
)}
...
<div className='mt-5 pt-3 mb-4 text-center'>
  {sendingEmail && (
    <button className='signUpButton rounded-littleSharp px-4 py-1 text-white'>
    <span className="spinner-border buttonCommonText spinner-border-sm me-2" role="status" aria-hidden="true"></span>
Sending...
    </button>
  )}
  
  {!sendingEmail && (
<button className='signUpButton buttonCommonText rounded-littleSharp px-4 py-1 text-white'
onClick={sendOtpOnEmail}>Confirm Email/ Phone</button>
  )}
...
{!otpBtn && (
<button className='disabledSignUpButton rounded-littleSharp px-4 py-1'>Confirm OTP</button>
)}
{otpBtn && (
<button className='signUpButton rounded-littleSharp px-4 py-1 text-white'
onClick={checkOtp}
...
<div className='mt-5 pt-3 mb-4 text-center'>
{!finalSignUpBtn &&(
<button className='disabledSignUpButton buttonCommonText rounded-littleSharp px-4 py-1'>Continue</button>
)}
{finalSignUpBtn &&(
<button className='signUpButton buttonCommonText rounded-littleSharp px-4 py-1 text-white'
onClick={()=>{
...
<div className="form-row">
<input type="text" value={houseNo} className="text_input" 
onFocus={element=>addInputStyle(element)} 
onChange={(element)=>{
  setHouseNo(element.currentTarget.value);
  addressValidate()
}}
onBlur={(element=>removeInputStyle(element))}
/>
<label className="form-row-field">House Number*</label>
</div>
...
<div className="form-row">
<input type="text" className="text_input" value={street} 
onFocus={element=>addInputStyle(element)}
onChange={(element)=>{setStreet(element.currentTarget.value); addressValidate()}} 
onBlur={(element=>removeInputStyle(element))}
/>
<label className="form-row-field">Street / Landmark</label>
</div>
...
<div className="form-row">
<input type="text" className="text_input" value={city} 
onFocus={element=>addInputStyle(element)}
onChange={(element)=>{setCity(element.currentTarget.value); addressValidate()}} 
onBlur={(element=>removeInputStyle(element))}
/>
<label className="form-row-field">City*</label>
</div>
...
<div className="form-row">
<input type="number" className="text_input" value={pincode} 
onFocus={element=>addInputStyle(element)}
onChange={(element)=>{setPincode(element.currentTarget.value); addressValidate()}} 
onBlur={(element=>removeInputStyle(element))}
/>
<label className="form-row-field">Pincode*</label>
</div>
...
<div className='mt-5 pt-3 mb-4 text-center'>
{!addressDone &&(
<button className='disabledSignUpButton buttonCommonText rounded-littleSharp px-4 py-1'>Sign Up</button>
)}
{addressDone &&(
<button className='signUpButton buttonCommonText rounded-littleSharp px-4 py-1 text-white'
